...
void nxp_init(const NxpConfig *cfg) {
  ...
  if (s_cfg.cursor_id < 0)        s_cfg.cursor_id = 8;   // <0 => default 8; 0 is allowed
...
/* Render space can change while the game runs -- on Switch, docking swaps the
 * surface between 1280x720 and 1920x1080. Event coordinates and the overlay are
 * both in render space, so a stale size means taps land in the wrong place and
 * the cursor is drawn at the wrong scale. The host calls this whenever its
 * surface size changes; the cursor keeps its RELATIVE position so it doesn't
 * jump to a corner across a dock transition. */
void nxp_set_screen(int w, int h) {
  ...
  /* Touches still down are held in the OLD space; rescale them too so a finger
   * that is mid-drag across the change doesn't teleport on its next UP. */
  const float rx = (float)w / (float)s_cfg.screen_w;
  const float ry = (float)h / (float)s_cfg.screen_h;
  for (int i = 0; i < 16; i++) {
    if (!s_touch_active[i]) continue;
    s_touch_x[i] *= rx;
    s_touch_y[i] *= ry;
  }
  ...
  s_cy = fy * (float)h;
  clamp_cursor();
  logf_("nxp: render space now %dx%d\n", w, h);
}
