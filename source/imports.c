...
static void gl_TexStorage2D_log(GLenum target, GLsizei levels, GLenum ifmt,
  ...
  { static unsigned seen; int watch = (bound >= 2300 && bound <= 2400); (void)watch;
    if (seen < 4) { seen++;
...
static EGLBoolean egl_SwapBuffers_log(EGLDisplay d, EGLSurface s) {
  g_swap_count++;
  int trace = (g_swap_count <= 3);                                /* boot only */
  if (trace) {
    EGLint w = -1, h = -1;
...
extern DynLibFunction dfu_imports_extra[];   /* dfu_imports_extra.c */
...
static void build_combined(void) {
  ...
  g_combined_n = (int)dynlib_numfunctions + unity_dynlib_numfunctions
               + (int)firebase_extra_numfunctions + (int)dfu_imports_extra_num;
  ...
  memcpy(g_combined + off, dfu_imports_extra,
         dfu_imports_extra_num * sizeof(DynLibFunction));
}
