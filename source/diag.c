...
typedef struct {
  ...
  volatile int          gc_paused;      /* WE paused it for a collection          */
} DiagThread;
 ...
 *
 * The collector asks for each thread to be suspended by signalling it. POSIX
 * signals are never delivered on Switch, so libc_shim's pthread_kill_gc() used
 * to just post the acknowledgement and let the thread keep running. The world
 * never stopped: the collector marked and swept while every other thread ran.
 *
 * The crash that finally proved it (AUDIT sec 28) had the audio thread read a
 * managed object header and get the class pointer back with BIT 0 SET -- the
 * collector's mark bit, present at the moment of the load and cleared again by
 * the time the crash dumper read the same word.
 *
 * The machinery to do this properly was already here: snapshot_thread() has
 * been pausing threads with svcSetThreadActivity() for the watchdog since long
 * before this. These two just expose it by pthread_t, which is what the GC
 * hands us.
 *
 * SAFETY. Never pause the caller (that is the collector itself, and pausing it
 * hangs the console). Never pause a thread that is already paused by us, so the
 * resume is symmetric. A failure is reported false so the caller still ACKS --
 * a collector waiting forever for an acknowledgement is worse than one that
 * runs with a thread it could not stop. */
int diag_pause_pthread(void *target_pthread) {
  pthread_t want = (pthread_t)target_pthread;
  for (int i = 0; i < DIAG_MAX_THREADS; i++) {
    DiagThread *t = &g_threads[i];
    if (!t->in_use || t->pth != want) continue;
    if (!t->handle || t->handle == threadGetCurHandle()) return 0;  /* never self */
    if (t->gc_paused) return 1;                                     /* already */
    if (R_FAILED(svcSetThreadActivity(t->handle, ThreadActivity_Paused))) return 0;
    t->gc_paused = 1;
    return 1;
  }
  return 0;                                   /* unknown thread: caller still acks */
}
...
/* Belt and braces: resume anything we still have paused. Called if the
 * collector ever loses track, so a missed restart cannot wedge the console. */
void diag_resume_all_gc_paused(void) {
  for (int i = 0; i < DIAG_MAX_THREADS; i++) {
    DiagThread *t = &g_threads[i];
    if (t->in_use && t->gc_paused) {
      t->gc_paused = 0;
      if (t->handle) svcSetThreadActivity(t->handle, ThreadActivity_Runnable);
    }
  }
}
