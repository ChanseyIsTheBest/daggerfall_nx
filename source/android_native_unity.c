...
#include "config.h"
#include "nx_data_root.h"  /* g_data_root */ /* screen_width / screen_height */
...
void android_native_update_mode(void){
  if (appletGetOperationMode() == AppletOperationMode_Console) { g_w = 1920; g_h = 1080; }  /* docked landscape   */
  else                                                         { g_w = 1280; g_h = 720;  }  /* handheld landscape */
...
void android_native_input_init(void){
  ...
  c.data_dir        = g_data_root;   /* runtime-resolved, see nx_data_root.h   */
