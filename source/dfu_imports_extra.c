/* dfu_imports_extra.c -- the MediaNDK / AImageReader / ALooper import surface.
 *
 * DAGGERFALL UNITY (Unity 2022.3.62f3): adopted from bouncemasters_nx
 * (bm_imports_extra.c, MIT), itself from killerbean_nx. The PvZ Fusion base
 * this port forks resolves 616 symbols; Daggerfall's libmain+libunity+libil2cpp
 * import 492 distinct symbols, of which 70 fall outside that set. This file
 * closes 68 of them; AImage_getWidth and strcasestr were added below.
 *
 * ORIGINAL HEADER FOLLOWS -- the analysis is engine-level and still applies.
 *
 * Adopted verbatim from killerbean_nx (imports_killerbean_extra.c, MIT); the
 * symbol groups below are engine-level and identical for Bouncemasters
 ...
 *   extern DynLibFunction dfu_imports_extra[];
 *   extern size_t         dfu_imports_extra_num;
...
static int     AImage_getWidth(void) { NOTSUP("AImage_getWidth"); return AMEDIA_ERROR_UNSUPPORTED; }
...
/* strcasestr -- glibc/bionic extension, absent from newlib. Real behaviour,
 * not a stub: libil2cpp calls it during locale/culture setup, and returning
 * NULL unconditionally would silently mis-resolve cultures. */
static char *nx_strcasestr(const char *h, const char *n) {
  if (!h || !n) return NULL;
  if (!*n) return (char *)h;
  size_t nl = strlen(n);
  for (; *h; h++)
...
DynLibFunction dfu_imports_extra[] = {
...
size_t dfu_imports_extra_num =
    sizeof(dfu_imports_extra) / sizeof(dfu_imports_extra[0]);
